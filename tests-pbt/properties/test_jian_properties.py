...
但用完全独立的 Python 实现(Hypothesis 生成器)与独立的执行路径(JPype 直调 JianJpypeBridge)。
...
# jian-core jar / 桥未构建时 skip 整套件(优雅降级,与 pandas 缺失 skip 同精神)。
# 因为 skip 守卫只在 import 期触发,若检查推迟到 _ensure_jvm()(测试体内),except 永不
# 生效,所以显式调 ensure_built() 让 skip 真正起作用
from jian_client import get_client, close_client, make_df, ensure_built  # noqa: E402
try:
    ensure_built()
except FileNotFoundError as _e:
    pytest.skip(f"jian 未构建,跳过跨语言 PBT({_e})", allow_module_level=True)


# === Hypothesis 生成器 ===

# 简单 df:strategy 生成 (id, v) 两列,id 在 [0, 50) 可能重复,v 在 [-100, 100)
# 边界注入:v 列混合普通值 [-100,100) 与 NaN(Hypothesis 不能同时
# 设 min_value 与 allow_nan=True,故用 one_of 显式组合)。NaN 经 jian_client._to_py 的 None 语义
# (NaN→null)转成缺失,等价 jian 的"缺失"语义。让 22 条性质在边界条件下也被检验。
...
def test_p2_sortBy_升序后单调不减(df):
    ...
    边界注入:v 含 NaN(NaN→null 后 Python 拿到 None)。
    None 与任何值的 >= 比较会抛 TypeError,所以跳过含 None 的相邻对
    (IEEE 754 NaN 不可比,与 Java 端 MR4/P2 同款处理)。
    """
    client = get_client()
...
def test_p5_head_行数等于minN(df, n):
    """head(n) 行数 = min(n, rowCount)。n 上限 200,覆盖 n > rowCount 边界。"""
    client = get_client()
    r = client.head(df, n)
...
def test_p10_groupBy_countSum等于原表行数(df):
    """P10:真正调 client.groupBy,断言各组 count 之和 == 原表行数(真蜕变关系)。

    边界注入:v 列含 NaN(NaN→null)。因为 count 聚合是"非空值计数"(pandas 语义),
    v 列含 null 会把计数吞掉,所以 count 必须用 **id 列**(id 列无 null,count == 组内行数),
    与 Java 端 P10 同款口径。
    """
    if not df["rows"]:
        return   # 空表跳过
...
def test_p13_ffill后首有效值之后无NaN(df):
    """ffill 后,每个填充值应等于前一个有效值(强断言:弱断言只验"无 NaN"会放过填充错值)。"""
    if not df["rows"]:
        return
    client = get_client()
    r = client.ffill(df)
...
def test_p21_colAdd_等于逐行加(df):
    """colAdd 后每行 sum == id + v。
    用 _dfs_no_nan(v 不含 NaN),避免 v 含 null 导致 OBJECT 列降级。
    """
    if not df["rows"]:
        return
    client = get_client()
...
def test_p23_assign_加列不改行数(df):
    ...
    空表也断言(Schema.inferColumn 有空表守卫,空表不会异常)。
    """
    client = get_client()
