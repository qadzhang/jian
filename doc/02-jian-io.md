...
- **标识符按需加引号(修复"中文列名一刀切拒绝"缺陷)**:`Sql.write/readTable` 对简单 ASCII 标识符
  ...
  真测:H2 + SQLite 默认套件(含中文表名/列名/`AA_a啊` 保真往返、注入式表名列名转义),PG 用例入 opt-in 套件。旧白名单死代码(assertSafe*)与误导注释、AGENTS §3.7.8 旧口径同步清除 —— 因为死防线与过时注释一旦被后来者误接线,会复活"中文标识符一刀切拒绝"的旧缺陷(CWE-561/1116)。
